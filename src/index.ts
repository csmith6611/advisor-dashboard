...
app.get(
  ...
  (
    req: Request<
      ...
      {},
      {},
      { ticker: string | undefined; sort: "asc" | "desc" | undefined }
    >,
    res
  ) => {
    ...
    if (ticker) {
      res.json(
        server_store.holdings_map.get(ticker) ?? {
          message: "No appropriate ticker found",
        }
      );
      return;
    }
...
app.get(
  ...
  (
    ...
    const relevant_advisor = server_store.advisor_data.find(
      (advisor) => advisor.id === id || advisor.name === name
    );
...
app.get(
  ...
  (
    ...
    if (account_number) {
      const relevant_account = server_store.account_data.find(
    ...
    if (sortValues) {
      const sorted_accounts_by_total_value = server_store
...
app.get("/custodian", (req: Request<{}, {}, {}, { name: string }>, res) => {
  ...
  if (name) {
    const return_obj = Object.fromEntries(
      Object.entries(custodians_object).filter(([custodian, _advisor]) => {
...
app.get(
  ...
  (
    ...
    res.json(
      filtered_securities.length > 0
        ? filtered_securities
        : { message: "No relevant securities found" }
    );
  }
);
